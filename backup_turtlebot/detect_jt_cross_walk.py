...
# 추가/
    def detect_crosswalk(self, cv_image):
        ...
        cw_roi = cv_image[int(height_c * 0.01):height_c, int(width_c * 0.15):int(width_c * 0.8)]
        ...
        cw_lines = cv2.HoughLinesP(cw_edges, 1, np.pi / 180, 50, minLineLength=25, maxLineGap=20)

        vertical_lines = []
        if cw_lines is not None:
            for line in cw_lines:
                x1, y1, x2, y2 = line[0]
                if abs(x2 - x1) < 10 and abs(y2 - y1) > 20:
        ...
        cv2.imshow("Crosswalk ROI", cw_roi)
        cv2.waitKey(1)
# /추가
