...
class HoughLaneVisualizer(Node):
    ...
    # 1. ROI 설정
        roi_start_y = int(height * 0.2) 
        roi_end_y = height
        roi_start_x = int(width * 0.05) # 이미지 좌측 5%는 제외
        roi_end_x = int(width * 0.95)
        # roi_start_x = 0 # 이미지 좌측 5%는 제외
        # roi_end_x = int(width * 0.85)
        # 최종 ROI 적용
    ...
    # 4. 검출된 선 필터링
        for line in lines:
            ...
            # 수평에 가까운 선은 제외
            # if abs(slope) < 0.05 or abs(slope) > 100.0:
            #     continue
            if abs(slope) > 112.0:
                continue
        # 중요
        ...
        if cx_left is not None and cx_right is not None:
            ...
            lane_center = int(cx_left + (width * 0.1))  # 오른쪽이 안 보이면 오른쪽 방향으로 보정
        elif cx_right is not None:
            lane_center = int(cx_right - (width * 0.1))  # 왼쪽이 안 보이면 왼쪽 방향으로 보정
        else:
