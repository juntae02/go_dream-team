...
class DetectLane(Node):
    ...
    def process_image(self, frame):
        ...
        # processed = self.auto_exposure(self.frame)

        # roi = self.frame[120:240, start:end]
        roi = self.frame
        ...
        _, binary = cv2.threshold(edge_img, 200, 255, cv2.THRESH_BINARY)

        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        dilated = cv2.dilate(binary, kernel, iterations=1)
        # cv2.imshow("Label Visualization", dilated )
        ...
        for i,h in enumerate(hierarchy[0]):
            if h[3] != -1:
                ...
                cv2.drawContours(dilated, contours, i , (0,255,0),2)

        cv2.imshow('binary2',dilated)

        final_center, lpos, rpos= self.get_stable_center_from_contours(child_contours)
        
        cv2.rectangle(self.frame, (lpos - 5, 215), (lpos + 5, 225), (0, 255, 0), 2)
        cv2.rectangle(self.frame, (rpos - 5, 215), (rpos + 5, 225), (0, 255, 0), 2)
        cv2.rectangle(self.frame, (final_center - 5, 215), (final_center + 5, 225), (0, 255, 0), 2)
        cv2.rectangle(self.frame, (final_center, 215), (final_center, 225), (0, 0, 255), 2)  

        processed_msg = self.cvBridge.cv2_to_imgmsg(self.frame, "bgr8")
        self.processed_image_pub.publish(processed_msg)
        return final_center
