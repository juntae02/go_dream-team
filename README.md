...
- **문제 상황 1: 빛 반사로 인한 차선 검출 오류 발생**
  ...
  - **해결** : 밝기 기반으로 차선을 검출하기 위해, Grayscale 변환으로 색상 의존도를 제거했습니다. 이후, Gaussian Blur와 모폴로지 Opening 연산을 사용하여 조명 조건에 의한 노이즈를 제거했습니다. 
    덕분에 Canny 엣지 결과, 노이즈와 얇은 선을 제거하고, 끊어진 선들을 연결해 안정적인 엣지를 추출했습니다.  
  👉 [밝기 기반 코드](https://github.com/juntae02/go_dream-team/blob/main/realistic_env/lane_detect/brightness_lane_detect.py#L108-L125)  
---

- **문제 상황 2: 차선이 아닌 직선 검출 발생**
  - **상황** : 허프 변환 후 불필요한 직선(짧은 선, 수평·수직선, 어두운 선 등)이 검출됐습니다.
