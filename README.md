...
- **문제 상황 1: 빛 반사로 인한 차선 인식 오류 발생**
  
  - **상황** : 색상 정보를 기반으로 차선 검출한 경우, 실제 차선이 아닌 빛 반사 지점이 차선으로 오인식되는 문제가 빈번하게 발생했습니다.  
  👉 [초기 차선 인식 이미지](https://github.com/juntae02/go_dream-team/blob/main/resource/test_img.png)
  
  - **원인** : 색상 정보가 실제 환경의 다양한 조명 조건(빛 반사, 그림자 등)에 매우 민감하게 반응하여 차선 인식의 안정성이 떨어졌습니다.
    
  - **해결** : 색상 정보의 불안정성을 해결하기 위해 밝기 정보에 집중하는 방식으로 전환했습니다. 이를 위해, 카메라의 노출 시간을 변경하여 이미지를 전반적으로 어둡게 조정했습니다. 그 결과, 빛 반사 영역의 밝기 정보가 과도하게 포화되는 것을 방지했고, 차선과 배경 간의 밝기 대비를 명확하게 확보할 수 있었습니다. 이후, 어둡게 조정된 이미지를 흑백으로 변환한 후, 특정 밝기 임계값을 설정하여 차선 영역만을 추출하는 알고리즘을 개발했습니다. 이 과정에서 모폴로지 연산 사용 ~~    
