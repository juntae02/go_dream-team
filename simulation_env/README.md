...
- **문제 상황 1: 보행자 접근 여부 판단 불가로 충돌 발생**
  ...
  - **상황** : 로봇이 멀리 있는 보행자를 '전방에 없음'으로 잘못 판단해 주행을 재개했고, 이후 보행자가 다가오면서 충돌이 발생했습니다.
...
- **문제 상황 3: 역광이나 조명 변화로 인한 색상 인식 오류 발생**
  ...
  - **상황** : Gazebo 가상 환경에서도 역광이나 조명이 바뀌면 장애물이 흐릿하게 보이거나 검출되지 않는 문제가 발생했습니다. 
  
  - **원인** : HSV 색공간의 고정된 임곗값을 사용했기 때문에 조명 조건이 변하면 마스크가 제대로 생성되지 않았습니다.
